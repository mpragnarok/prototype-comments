/* pc.js 19c54c3 2026-08-04T16:15:02Z */
const STYLES = `
/* ── prototype-comments ──────────────────────────── */
...
function resolveShortcutByCode(code) {
...
function polylineDist(p, points) {
...
function pointNearPolyline(p, points, tol) { return polylineDist(p, points) <= tol; }
...
function objHitDist(o, p, tol, ends) {
  if (o.tool === 'arrow' || o.tool === 'line') {
    const e = ends || o.geom;
...
function pointHitsObject(o, p, tol, ends) { return objHitDist(o, p, tol, ends) !== Infinity; }
...
function makeDrawObject({ id, tool, geom, style, text, imageRef, endAnchors, screenId, hidden, archivedAt } = {}) {
...
function isTombstoned(tombstones, id) {
  return !!(tombstones && tombstones[id]);
}

// 從帶 .id 的項目陣列（objects / notes / docs）濾掉已墓碑者。回傳新陣列。
function filterTombstoned(items, tombstones) {
...
function compactTombstones(tombstones, now = Date.now(), maxAgeMs = TOMBSTONE_MAX_AGE_MS) {
  const out = {};
...
export const PC_VERSION = '19c54c3';
...
export async function initPrototypeComments(opts = {}) {
  ...
  function anchorUnderPoint(clientX, clientY) {
    ...
    if (target.closest('[class^="pc-"], [class*=" pc-"], [id^="pc-"]')) return {}; // 標注層自己的 UI 不算
    ...
    if (currentUser) {
      ...
      submitBtn.onclick = async () => {
        ...
        await store.save({
          ...
          authorPhoto: currentUser.photoURL || '',
          resolved: false,
          // 回覆也是新訊息（使用者常在這裡補充「還有這個也怪怪的」），一樣要進待辦。
          ...(collectToTasks ? { filedAt: null } : {}),
        });
