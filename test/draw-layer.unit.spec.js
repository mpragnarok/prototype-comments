...
test('pointHitsObject: arrow 用線段距離；外框 rect 用邊線、填充 rect 用內部', () => {
  ...
  const rect = makeDrawObject({ tool: 'rect', geom: { x: 0, y: 0, w: 20, h: 20 } }); // fill:none 預設
  assert(pointHitsObject(rect, { x: 0.5, y: 10 }, 2), '外框邊上 → 命中');
  assert(!pointHitsObject(rect, { x: 10, y: 10 }, 2), '外框 rect 內部空白 → 不命中（讓底下物件可選）');
  const filled = makeDrawObject({ tool: 'rect', geom: { x: 0, y: 0, w: 20, h: 20 }, style: { fill: '#f00' } });
  assert(pointHitsObject(filled, { x: 10, y: 10 }, 2), '填充 rect 內部 → 命中');
});
test('pointHitsObject: 外框 ellipse 用外框（中心不命中、外框上命中）', () => {
...
test('resolveShortcutByCode: 用 e.code 對應工具（注音/IME 不影響）', () => {
  ...
  eq(resolveShortcutByCode('Digit2'), 'rect');
  eq(resolveShortcutByCode('KeyV'), 'select');
  eq(resolveShortcutByCode('KeyA'), 'arrow');
  eq(resolveShortcutByCode('Backspace'), null);
});
