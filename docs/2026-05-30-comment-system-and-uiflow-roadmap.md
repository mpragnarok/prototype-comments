...
- **✅ B7 全部留言面板 inline thread 不重複主留言**（2026-05-31 使用者回報「點開後第一項也是 thread 主留言」）：根因＝panel item 已顯示 root 的作者＋摘要，展開時 `renderInlineNoteThread` 又把該 noteKey 的所有 root 用完整 `buildCommentItem` 渲染一次 → 第一張卡與上方重複。**修法**：(1) `buildCommentItem` 加 `compact` 參數——略過頭像／作者／內容，只留 reactions＋操作（resolve／回覆／刪除），編輯鈕在 compact 下不顯示（避開 txtEl 依賴）；(2) `renderInlineNoteThread` 改成只渲染「該 root 的 sub-thread」：root 以 `compact:true` 出（不重複內容）＋回覆 scope 到 `parentId === root.id`。e2e source-check 鎖 `compact:true` + `x.parentId === root.id`（legacy spec 32 passed）；npm test（mock 7/7 + visual identical）綠。
- **✅ B8 reaction「看誰按了」確認已上線**（2026-05-31 使用者問「是不是沒辦法 hover 看誰點的／沒補 roadmap」）：查證＝B5（§2.A）已實作＋e2e 兩路徑綠＋commit 進 `dist/pc.js`＋`main` 已 push＋jsDelivr CDN 實測含 `bindReactionChip`/`showReactionUsers`。**功能在且 live**：桌機 hover **reaction chip（如 👍2 膠囊，非 emoji 選擇器）**→名單 popover；手機長按 ≥400ms chip→名單。前提不成立，無需新開發；僅需說明互動位置（hover 在已存在的 chip）。
- **✅ C4 ui-flow 右側 info-panel scroll bar**（2026-05-31 使用者回報「右邊說明要能滑，不然沒法邊看 dev note 邊看設計稿」）：根因＝`body{min-height:100vh}` 非固定高度 → `.main`(flex:1) 被內容撐高、`.info-panel` 雖 `overflow-y:auto` 但無「有界高度」故不出捲軸。**修法**：桌機 `body{height:100vh;overflow:hidden}` 鎖高度 → header(flex-shrink:0)+main(flex:1) 剛好填滿，三欄（sidebar／stage／info-panel）各自在有界高度內捲；`@media(max-width:767px)` 加 `body{height:auto;overflow:visible;min-height:100vh}` 讓手機堆疊版恢復整頁捲。套用 `ui-flow-template.html`（master）+ `tournament-ui-flow.html`（live）。⚠️ 需 deploy 後實機目視確認。
...
- **✅ C5 pc.js 自動更新（免手動 hot reload）— DONE 2026-06-01**（使用者：「每次更新 pc.js 後都要 hot reload，有沒有方法讓使用者不用自己按？」）：**根因**＝jsDelivr 給 pc.js 的 `cache-control: max-age=604800`（瀏覽器快取 7 天）→ pc.js 改版後一般「重新整理」仍吃舊版、得 hard reload。**修法兩段**：(1) **cache-bust import** —— 改 `const {initPrototypeComments}=await import(PC_URL+'?t='+Date.now())`，每次載入抓最新（jsDelivr 邊緣仍 HIT、不變慢；驗證 query 不影響 ETag）；(2) **ETag 輪詢自動 reload** —— 每 12s `fetch(PC_URL,{method:'HEAD',cache:'no-store'})` 比對 ETag（`access-control-expose-headers:*` 可跨域讀），一改版就 `location.reload()`，並用 `sessionStorage('pc-reload-screen')` 還原當前畫面；**正在打字**（focus 在有內容輸入框）則改顯示「點此重新整理」teal 膠囊、不打斷。套用 `ui-flow-template.html`（master）+ `tournament-ui-flow.html`（live，CDN pc.js）+ `03-generate.js` 的 `pcInit`（本地 `./pc.js`，給 vitallink 等生成 flow）。各檔 module/main `node --check` 通過、已部署 jubo design docs。
